...
SUMMARY:  Dcase 2016 Task 1. Scene classification
          Training time: 33 s/epoch. (Tesla M2090)
          test acc: 67% +- ?, test frame acc: 55% +- ? after 10 epoches. 
...
from Hat.layers.cnn import Convolution1D
from Hat.layers.rnn import SimpleRnn, LSTM, GRU
from Hat.layers.pool import GlobalMeanTimePool, MaxPool1D
...
md.add( Convolution1D( n_outfmaps=50, len_filter=3, act='tanh' ) )    # 1d cnn
md.add( MaxPool1D( len_pool=2 ) )             # down sample across time axis
